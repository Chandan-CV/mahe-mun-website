<script lang="ts">
	...
	function validateNumberOfDel() {
		if (!/^\d+$/.test(numDelegates)) {
			formErrors['numDelError'] = [true, 'Enter an integer'];
		} else if (Number(numDelegates) < 10) {
			formErrors['numDelError'] = [true, 'Minimum 10 delegates required'];
		} else {
			formErrors['numDelError'] = [false, ''];
		}
	}
